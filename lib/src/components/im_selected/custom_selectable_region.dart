...
/// with a [CustomSelectableRegion] widget enables selection within that subtree (for
...
///
/// A [CustomSelectableRegion] widget requires configuration; in particular specific
/// [selectionControls] must be provided.
///
/// The [SelectionArea] widget from the [material] library configures a
/// [CustomSelectableRegion] in a platform-specific manner (e.g. using a Material
/// toolbar on Android, a Cupertino toolbar on iOS), and it may therefore be
/// simpler to use that widget rather than using [CustomSelectableRegion] directly.
///
/// ## An overview of the selection system.
///
/// Every [Selectable] under the [CustomSelectableRegion] can be selected. They form a
/// selection tree structure to handle the selection.
///
/// The [CustomSelectableRegion] is a wrapper over [SelectionContainer]. It listens to
...
///
/// The [SelectionContainer] created by [CustomSelectableRegion] is the root node of a
...
/// a [CustomSelectableRegion], consider wrapping that group of widgets using
...
/// wrap part of the subtree with another [CustomSelectableRegion]. The selection of the
...
///
/// See also:
///  * [SelectionArea], which creates a [CustomSelectableRegion] with
...
class CustomSelectableRegion extends StatefulWidget {
  /// Create a new [CustomSelectableRegion] widget.
  ///
  ...
  ///
  /// By default, [CustomSelectableRegion]'s [TextMagnifierConfiguration] is disabled.
  ///
  ...
  ///
  /// For example, [CustomSelectableRegion] uses this to generate the default buttons
  /// for its context menu.
  ///
  /// See also:
  ///
  /// * [CustomSelectableRegionState.contextMenuButtonItems], which gives the
...
/// State for a [CustomSelectableRegion].
class CustomSelectableRegionState extends State<CustomSelectableRegion> with TextSelectionDelegate implements SelectionRegistrar {
  ...
  bool get contextMenuAnchorsCan {
    if(_selectionDelegate.value.startSelectionPoint==null)return false;
    return true;
  }
  ...
  /// See also:
  ///
  /// * [CustomSelectableRegion.getSelectableButtonItems], which performs a similar role,
  ...
  List<ContextMenuButtonItem> get contextMenuButtonItems {
    return CustomSelectableRegion.getSelectableButtonItems(
